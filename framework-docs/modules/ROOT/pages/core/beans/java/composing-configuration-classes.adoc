...
In this series of examples, we create a `@Configuration` class named `AppConfig2` and
...
`@Configuration` annotation and processes the `@Bean` methods declared in `AppConfig2`
properly.

The following example shows the `AppConfig2` configuration class in Java and Kotlin:
...
NOTE: In the `system-test-config.xml` file, the `AppConfig2` `<bean/>` does not declare an `id`
