...
// <p>{@link DefaultResourceLoader} 是一个独立实现，可在 ApplicationContext 之外使用，也可由 {@link ResourceEditor} 使用。
// <p>在 ApplicationContext 中运行时，可以使用特定上下文的资源加载策略从字符串填充类型为 {@code Resource} 和 {@code Resource[]} 的 Bean 属性。
public interface ResourceLoader {
	...
	// <p>该句柄应始终是可重用的资源描述符，以允许多次 {@link Resource#getInputStream()} 调用。
	// <p><ul>
	// <li>必须支持完全限定的 URL，例如 “file:C:/test.dat”。
	// <li>必须支持类路径伪 URL，例如 “classpath:test.dat”。
	// <li>应支持相对文件路径，例如 “WEB-INF/test.dat”。 （这将特定于实现，通常由 ApplicationContext 实现提供。）
	// </ul>
	// <p>请注意，{@code Resource} 句柄并不意味着资源已存在；您需要调用 {@link Resource#exists} 来检查是否存在。
	// @param location 资源位置
	// @return 相应的 {@code Resource} 句柄（绝不会返回 {@code null}）
	...
	// <p>需要直接访问 {@code ClassLoader} 的客户端可以使用 {@code ResourceLoader} 以统一的方式进行访问，而无需依赖线程上下文 {@code ClassLoader}。
	// @return {@code ClassLoader}（如果系统 {@code ClassLoader} 也无法访问，则返回 {@code null}）
	@Nullable
