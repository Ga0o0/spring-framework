...
// 可以检查传入的 {@code ResourceLoader}（例如，在上下文中运行时通过 {@link org.springframework.context.ResourceLoaderAware}
// 传入的 {@link org.springframework.context.ApplicationContext}）是否也实现了此扩展接口。
//
// <p>{@link PathMatchingResourcePatternResolver} 是一个独立的实现，可在 {@code ApplicationContext} 之外使用，
// 也可由 {@link ResourceArrayPropertyEditor} 用于填充 {@code Resource} 数组 bean 属性。
//
// <p>可与任何类型的位置模式一起使用 -例如，{@code "/WEB-INF/*-context.xml"}。
// 但是，输入模式必须与策略实现匹配。此接口仅指定转换方法，而不指定具体的模式格式。
//
// <p>此接口还为模块路径和类路径中所有匹配的资源定义了一个 {@value #CLASSPATH_ALL_URL_PREFIX} 资源前缀。
// 请注意，资源位置也可能包含占位符，例如 {@code "/beans-*.xml"}。模块路径或类路径中的 JAR 文件或不同目录可以包含多个同名文件。
public interface ResourcePatternResolver extends ResourceLoader {
	 ...
	 */
	// 为类路径中所有匹配资源添加伪 URL 前缀：{@code "classpath*:"}。
	// <p>这与 ResourceLoader 的 {@code "classpath:"} URL 前缀不同，它会检索给定路径的所有匹配资源。
	// 例如，要在所有已部署的 JAR 文件的根目录中找到所有“beans.xml”文件，可以使用位置模式 {@code "classpath*:/beans.xml"}。
	// <p>从 Spring Framework 6.0 开始，{@code "classpath*:"} 前缀的语义已扩展，不仅包含类路径，还包含模块路径。
	...
	// <p>应尽可能避免指向同一物理资源的重叠资源条目。结果应具有固定语义。
	// @param locationPattern 要解析的位置模式
	// @return 相应的 {@code Resource} 对象
