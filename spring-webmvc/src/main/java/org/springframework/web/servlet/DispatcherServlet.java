...
public class DispatcherServlet extends FrameworkServlet {
	...
	private void processDispatchResult(HttpServletRequest request, HttpServletResponse response,
		...
		// Did the handler return a view to render? --> 译文：handler 是否返回了要渲染的视图？
	...
	protected ModelAndView processHandlerException(HttpServletRequest request, HttpServletResponse response,
		...
		if (this.handlerExceptionResolvers != null) {
			for (HandlerExceptionResolver resolver : this.handlerExceptionResolvers) {
				// 执行 HandlerExceptionResolver#resolveException() 方法
		...
		if (exMv != null) {
			...
			if (!exMv.hasView()) {
				// 执行 RequestToViewNameTranslator#getViewName(HttpServletRequest) 方法
				String defaultViewName = getDefaultViewName(request);
				if (defaultViewName != null) {
	...
	private void restoreAttributesAfterInclude(HttpServletRequest request, Map<?, ?> attributesSnapshot) {
		...
		// on the Enumeration when removing attributes. --> 译文：需要将其复制到单独的集合中，以避免在删除属性时对枚举产生副作用。
		...
		// Add attributes that may have been removed --> 译文：添加可能已被移除的属性
		attrsToCheck.addAll((Set<String>) attributesSnapshot.keySet());

		// Iterate over the attributes to check, restoring the original value
		// or removing the attribute, respectively, if appropriate. --> 译文：遍历属性进行检查，如果合适，则分别恢复原始值或删除属性。
