...
public abstract class TransactionManagementConfigUtils {
	 ...
	 */
	// 内部管理事务顾问的 bean 名称（当 mode == PROXY 时使用）。
	 ...
	 */
	// 内部管理事务方面的 bean 名称（当 mode == ASPECTJ 时使用）。
	 ...
	 * @since 5.1
	 */
	// 内部管理的 JTA 事务切面的 Bean 名称（当 mode == ASPECTJ 时使用）。
	 ...
	 * @since 5.1
	 */
	// JTA 的 AspectJ 事务管理 @{@code Configuration} 类的名称。
